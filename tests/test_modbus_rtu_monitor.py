...
def test_monitor_driver_returns_bad_result_for_unobserved_configured_tag():
    ...
    device = DeviceSpec(
        ...
        enabled=True,
        poll_interval_ms=1000,
        connection={"port": "COM3", "baudrate": 9600, "parity": "E", "stopbits": 1, "bytesize": 8, "capture_wait_s": 1},
    )
    tags = [
        TagSpec(name="slave_1_speed", address=4099, function="holding_register", data_type="uint16", unit_id=1),
        TagSpec(name="slave_2_speed", address=4099, function="holding_register", data_type="uint16", unit_id=2),
    ...
    assert [(result.name, result.quality, result.error) for result in results] == [
        ("slave_1_speed", "good", None),
        ("slave_2_speed", "bad", "tag was not observed in captured Modbus RTU traffic"),
    ]
